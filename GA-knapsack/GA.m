...
nvar=length(v);    % number of variable



npop=100;         % number of population

maxiter=500;      % max of iteration
...
for i=1:npop
   pop(i).par=randi([0 1],1,nvar);
   pop(i).fit=fitness(pop(i).par,v,w,W);
...
for iter=1:maxiter
   ...
   crosspop=crossover(crosspop,pop,nvar,ncross,v,w,W);
   ...
   mutpop=mutation(mutpop,pop,nvar,v,w,W,nmut,npop);
